...
public class AvlTree<ValueType extends Comparable<? super ValueType> > {
    ...
    private boolean remove(ValueType value, BinaryNode<ValueType> currentNode) {
        ...
        boolean succes = true;
        if (comparaison > 0 && currentNode.right == null || comparaison < 0 && currentNode.left == null) {
            return false;
        }

        if (comparaison > 0) {
            succes = remove(value,currentNode.right);
        }
        else if(comparaison < 0) {
            succes = remove(value,currentNode.left);
        } else if (currentNode.left != null && currentNode.right != null) {
            BinaryNode<ValueType> temp = findMin(currentNode.right);
