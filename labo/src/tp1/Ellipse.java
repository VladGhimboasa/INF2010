...
public class Ellipse extends BaseShape {
    ...
    public Ellipse(Double widthRadius, Double heightRadius) {
        super();
        int widthRadiusInt = widthRadius.intValue();
        int heightRadiusInt = heightRadius.intValue();

        for(int i = 0; i< widthRadiusInt; i++ ) {
        ...
        for(int i = 0; i<heightRadiusInt; i++) {
            this.add(new Point2d((Math.sqrt(((1-(Math.pow((-heightRadius/2+i),2)/(Math.pow(heightRadius/2, 2))))*Math.pow(widthRadius/2, 2)))),-(heightRadius/2)+i));
            this.add(new Point2d((-Math.sqrt(((1-(Math.pow((-heightRadius/2+i),2)/(Math.pow(heightRadius/2, 2))))*Math.pow(widthRadius/2, 2)))),-(heightRadius/2)+i));
        }
